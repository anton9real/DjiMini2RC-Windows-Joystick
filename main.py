# Modified by Anton (2026)
# Changed Linux uinput interface to Windows vgamepad interface.
# Original project under Apache License 2.0.

import serial, argparse, binascii, struct, sys, time, vgamepad as vg
...
def threaded_function():
    while(True):
        ...
        if st['b2']: device.press_button(button=vg.XUSB_BUTTON.XUSB_GAMEPAD_B)
        else: device.release_button(button=vg.XUSB_BUTTON.XUSB_GAMEPAD_B)
        
        if st['b3']: device.press_button(button=vg.XUSB_BUTTON.XUSB_GAMEPAD_X)
        else: device.release_button(button=vg.XUSB_BUTTON.XUSB_GAMEPAD_X)
